...
export type SingleChar<T extends string = string> = T extends `${infer First}${infer Rest}`
	? Rest extends ''
		? First
		: never
	: never

export type ObjectToArray<T extends Record<string, Any>, Key extends string = 'type'> = {
	[K in keyof T]: T[K] & { [k in Key]: K };
}[keyof T][]
